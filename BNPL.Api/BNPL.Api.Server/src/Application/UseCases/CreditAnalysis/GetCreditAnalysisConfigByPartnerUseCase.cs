...
namespace BNPL.Api.Server.src.Application.UseCases.CreditAnalysis
{
    ...
    {
        public async Task<Result<IEnumerable<CreditAnalysisConfigDto>, Error>> ExecuteAsync(Guid partnerId)
        {
            var list = await creditAnalysisRepository.GetAllByPartnerAsync(partnerId);

            return Result<IEnumerable<CreditAnalysisConfigDto>, Error>.Ok(list.Select(c => c.ToDto()));
        }
    }
}